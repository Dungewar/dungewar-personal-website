...
let bombsSpawned: boolean = false;
...
function numNeighbors(x: number, y: number): number {
    ...
    for (let col = x - 1; col <= x + 1; col++) {
        for (let row = y - 1; row <= y + 1; row++) {
            if (inBounds(col, row) && board[col][row].bomb) numNeighbors++;
        }
    }
    return numNeighbors;
...
tilemap.onload = () => {
    ...
    function reloadLevel(size: number) {
    ...
    function spawnBombs(excludedX: number, excludedY: number, bombCount: number): void {
        const size = board.length;
        if (bombCount <= 0 || bombCount >= size * size - 9) throw new Error("Invalid bomb count " + bombCount);
