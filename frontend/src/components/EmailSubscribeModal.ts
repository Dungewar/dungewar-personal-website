...
class EmailSubscribeModal extends HTMLElement {
    ...
    private syncOpen() {
        ...
        if (isOpen) setTimeout(() => this.inputEl.focus(), 10);
    }
    ...
    private setMessage(text: string, kind: 'info'|'error'|'success' = 'info') {
    ...
    `;
        ...
        overlay.addEventListener('click', (e) => {
            if (e.target === overlay) this.hide(); // click outside
        });
