...
`sudo` is your hidden power against daemons (sorry Windows users (it's difficult to do sudo on Windows)). Standing for '`su`peruser `do` X', it's a call to the heavens, for the superuser - that being root (the guy in charge) - to execute any command you desire, such as executing a daemon. However, simply running `sudo systemctl kill 70294` (70294 being the process ID of the evil server) did not work, as it keeps on getting revived. The question now is: who's reviving it? Turns out there's an easy way to find out.
### The permament execution
`systemctl status 70294` gave us the culprit. All along, it was PM2.
![img_3.png](img_3.png)
## The betrayal
You might ask: how have I not noticed this sooner? Well in my defence, `pm2 status` gave me nothing, just my backend that was restarting over and over. This means one of two things:
1. PM2 is lying to me (possible, I don't trust it all that much)
2. Another PM2 exists

`btop` tells me the whole story...
![img_5.png](img_5.png)
There was another PM2. And it wasn't just any PM2, it was the **PM2 God Daemon** running not as dungewar, but as **root** (DUN DUN DUUUUN!).

How is this possible? I never ran `sudo pm2` to give birth to this abomination...
<br><br><br><br>
at least not explicitly.
## The mistake
When initializing the backend of the website, I have a 2 script combo `update-project.sh` and `update-project-entry.sh` that updates from github, redownloads node packages, sends out update emails, and importantly... restarts the backend. The script told me that some things were not permitted, that it needs root, so I foolishly gave it.
![img_6.png](img_6.png)
The likely cause of this is that when first setting it up, I ran `npm install` as root, so the `package.json` and `package-lock.json` were owned by root, so my script needed root to access them.
![img_8.png](img_8.png)

Within the `update-project.sh` script, I start PM2. Because the whole thing being ran as root, it started PM2 as root, and because of that just checking `pm2 status` did not show anything. If I had ran `sudo pm2 status` it would tell a different story, the story of the hidden server that was eating up port 4000.
## The solution
The Final Sol- no I mean the Good Proper At The End Of The Thing Solution was to unleash sudo upon the root-ran PM2. The thing about root is that it can do anything, will kill whoever you want it to, even its own spawn. With the help of my good friend `btop` (check out at www.btop.com/dungewar for 98% off your fist purchase over \$10! (that is \$(10!))).

So I ran `sudo btop`, went to the PM2 God Daemon ran by root, and after contemplating over the long hard battle, sent SIGKILL
![img_7.png](img_7.png)
## Aftermath
With SIGTERM, programs have time to accept the signal, shut down processes cleanly, then exit on their own accord. SIGKILL is not like that. As soon as the kernel sees the SIGKILL, it starts tearing down the walls of the root PM2 God Daemon, stoping all its calls, removing its system memory, and delocating all the taken threads without as much as a single whisper to it. At the end, nothing remains of it. It's an echo long gone, and along with it, its child, the `sudo node server.js` that was occupying port 4000 disappears.
